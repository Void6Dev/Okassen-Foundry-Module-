...
<ul>
  ...
  <li><code>"transform"</code> — превращение носителя в другого актёра <strong>без клона
      в сайдбаре</strong>. Цель и настройки берутся из
      <code>extraFlags.okassen.transform</code>:
      <pre><code>"extraFlags": { "okassen": { "transform": {
  "target": "Actor.xxxxxxxxxxxxxxxx",
  "preset": "polymorph",
  "toggle": true
} } }</code></pre>
      Модуль отвязывает токен носителя и превращает именно ТОКЕН (форма пишется в его
      дельту), поэтому мировой актёр и лист персонажа не меняются. Повторное использование
      возвращает исходный облик (отдельный обработчик — <code>"revert"</code>). В новой форме
      автоматически появляется черта «Вернуть облик» — возврат в один клик с листа зверя
      (отключается через <code>"returnItem": false</code>).
      Нужен токен на сцене; игрокам нужна настройка dnd5e «Разрешить игрокам превращения».</li>
</ul>
<p>Обработчики общего назначения — то, ради чего обычно пишут макросы. Конфиг каждого
лежит во флаге с тем же именем (строка = краткая запись главного поля). Действия,
на которые у игрока нет прав (эффект на чужого актёра, токен на сцене), модуль
отправляет активному ведущему — он перечитывает конфиг из самого предмета и
выполняет их сам (настройка «Выполнять действия игроков от имени ведущего»):</p>
<ul>
  <li><code>"chatCard"</code> — сообщение в чат. Конфиг: строка с текстом либо
      <code>{ "content", "flavor", "whisper": "gm" }</code>. В тексте подставляются
      <code>{item}</code>, <code>{actor}</code>, <code>{target}</code>:
      <pre><code>"extraFlags": { "okassen": { "chatCard": "{actor} вскрывает печать {item}." } }</code></pre></li>
  <li><code>"applyEffect"</code> — наложить эффект предмета на <strong>выбранные цели</strong>
      (или на носителя, если целей нет). Конфиг:
      <code>{ "effect": "Имя или id", "to": "targets" | "self" | "both", "duration": {...},
      "stack": false }</code>. Без <code>effect</code> берутся все эффекты предмета с
      <code>"transfer": false</code> — те, что и предназначены целям. Повторное применение
      не копит дубли: прежние эффекты от этого же предмета снимаются (<code>"stack": true</code>
      отключает это). Наложить эффект на чужого актёра игрок не может — модуль честно
      предупредит, что нужны права.</li>
  <li><code>"toggleEffect"</code> — переключить эффект носителя
      (<code>{ "effect": "Имя" }</code>) или состояние-condition
      (<code>{ "status": "prone" }</code>).</li>
  <li><code>"rollTable"</code> — бросок по таблице:
      <code>{ "table": "Имя или uuid", "roll": "1d20", "rollMode": "gmroll" }</code>.</li>
  <li><code>"summon"</code> — призвать актёра токеном рядом с носителем:
      <code>{ "actor": "Actor.xxxx", "count": 2, "name": "Волк {actor}", "distance": 1,
      "disposition": 1 }</code>. Токены создаёт только ведущий.</li>
  <li><code>"macro"</code> — вызвать макрос по имени или uuid:
      <code>{ "macro": "Мой макрос", "args": { "power": 3 } }</code>. Внутри макроса
      доступны и контекст (<code>item</code>, <code>actor</code>, <code>trigger</code>),
      и поля <code>args</code>.</li>
</ul>
...
<ul>
  ...
  <li><code>onRest</code> — носитель завершил отдых; в контексте <code>restType</code>
      (<code>"short"</code> / <code>"long"</code>) и полный результат отдыха <code>rest</code>;</li>
  <li><code>onDamaged</code> / <code>onHealed</code> — носителю уменьшили / восстановили хиты;
      в контексте <code>delta</code> (на сколько), <code>hp</code> и <code>previousHp</code>;</li>
  <li><code>onCombatStart</code> / <code>onCombatEnd</code> — бой с участием носителя начался
      или завершён.</li>
</ul>
<p>У <strong>актёров</strong> (в <code>_forge</code> актёра) поддерживается всё, кроме хуков
самого предмета (<code>onEquip</code>, <code>onCreate</code>, <code>onDelete</code>): ходовые,
отдых, изменение хитов и бой — поведение НИПа без предмета-носителя.
Контекст обработчика: <code>{ item, actor, trigger, ... }</code> (<code>item = null</code>
для хуков актёра). Ходовые хуки, отдых, хиты и бой обрабатывает активный ведущий — сработают
они один раз, а не у каждого клиента.</p>
...
<h3><code>_forge.sourceId</code> — якорь повторного импорта</h3>
<p>Необязательная строка — ваш собственный идентификатор документа
(<code>"okassen.copper-staff"</code>). При повторном импорте модуль ищет документ
сначала по нему, и только потом по имени и типу — поэтому переименование документа
в мире не мешает обновлению.</p>
<p>Когда документ найден, диалог предлагает три ответа:</p>
<ul>
  <li><strong>Обновить на месте</strong> — документ остаётся тем же: uuid, папка, права
      и место в инвентаре сохраняются, меняется только содержимое. Эффекты, созданные
      модулем, пересобираются с теми же <code>_id</code> (привязки активностей не рвутся),
      а эффекты, добавленные руками, остаются. Вложения пересоздаются, если в JSON есть
      блок <code>nested</code>. У актёров предметы из <code>items</code> обновляются тем же
      способом; предмет, убранный из JSON, остаётся на актёре — удалите его вручную.</li>
  <li><strong>Заменить</strong> — старый документ удаляется, создаётся новый (ссылки на
      старый uuid рвутся);</li>
  <li><strong>Создать копию</strong> — рядом появляется второй документ.</li>
</ul>
<p><strong>Важно:</strong> обновление — это пересборка, а не слияние: блок <code>system</code>
заменяется целиком. Поле, которого нет в JSON, вернётся к умолчанию системы. Правьте
документ в JSON, а не в листе, — тогда повторный импорт всегда даёт ожидаемый результат.</p>
<p>Ответ по умолчанию задаётся в настройках модуля («Что делать с дубликатом»); пакетный
импорт применяет его молча, без вопросов. Любое обновление попадает в «Историю» —
откат вернёт документ к состоянию до импорта.</p>

<h3>Вкладка «Исходники»: что разошлось с JSON</h3>
<p>Модуль хранит исходный JSON каждого созданного документа в
<code>flags.okassen.source</code>. Вкладка <strong>«Исходники»</strong> сравнивает документы
с их исходниками и показывает те, где содержимое разошлось: обычно это правки,
сделанные руками в листе, — следующий импорт их перезапишет.</p>
<ul>
  <li><strong>Показать diff</strong> — что именно отличается (то же доступно из контекстного
      меню документа в сайдбаре: «Сравнить с исходником»);</li>
  <li><strong>Пересобрать</strong> — переимпортировать документ из его исходника;</li>
  <li><strong>Пересобрать всё</strong> — то же для всех документов модуля, одной записью
      в истории: «Отменить» вернёт мир к состоянию до пересборки.</li>
</ul>
